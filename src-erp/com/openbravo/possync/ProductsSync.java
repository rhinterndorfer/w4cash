...
public class ProductsSync implements ProcessAction {
	...
	public ProductsSync(DataLogicSystem dlsystem, DataLogicIntegration dlintegration, DataLogicSales dlsales,
			String warehouse) {
		...
		try {
			...
			if (products.length > 0) {
				...
				for (Product product : products) {
					...
					TaxInfo t = new TaxInfo(product.getTax().getId(), product.getTax().getName(), tc.getID(),
							new Date(Long.MIN_VALUE), null, null, product.getTax().getPercentage() / 100, false, 0);
					...
					p.setCode(product.getEan() == null || product.getEan().equals("") ? product.getId()
							: product.getEan());
			...
			if (products.length == 0 && customers.length == 0) {
				...
				return new MessageInf(MessageInf.SGN_SUCCESS, AppLocal.getIntString("message.syncproductsok"),
						AppLocal.getIntString("message.syncproductsinfo", products.length, customers.length));
			}
