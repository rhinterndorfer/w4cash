...
public class JCalendarDialog extends javax.swing.JDialog {
	...
	public JCalendarDialog(AppView app, java.awt.Frame parent, boolean modal) {
		super(parent, modal);
	...
	/** Creates new form JCalendarDialog */
	public JCalendarDialog(AppView app, java.awt.Dialog parent, boolean modal) {
		super(parent, modal);
	...
	public static Date showCalendarTimeHours(AppView app, Component parent, Date date) {
		return internalCalendarTime(app, parent, date == null ? DateUtils.getToday() : date, true);
	}

	public static Date showCalendarTime(AppView app, Component parent, Date date) {
		return internalCalendarTime(app, parent, date == null ? DateUtils.getTodayMinutes() : date, true);
	}

	public static Date showCalendar(AppView app, Component parent, Date date) {
		return internalCalendarTime(app, parent, date == null ? DateUtils.getTodayMinutes() : date, false);
	}

	private static Date internalCalendarTime(AppView app, Component parent, Date date, boolean bTimePanel) {

		Window window = getWindow(parent);

		JCalendarDialog myMsg;
		if (window instanceof Frame) {
			myMsg = new JCalendarDialog(app, (Frame) window, true);
		} else {
			myMsg = new JCalendarDialog(app, (Dialog) window, true);
		}
	...
	private void jcmdOKActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_jcmdOKActionPerformed
		...
		if (myTime == null) {
			dateresult = new GregorianCalendar(date1.get(GregorianCalendar.YEAR), date1.get(GregorianCalendar.MONTH),
			...
			dateresult = new GregorianCalendar(date1.get(GregorianCalendar.YEAR), date1.get(GregorianCalendar.MONTH),
					date1.get(GregorianCalendar.DAY_OF_MONTH), date2.get(GregorianCalendar.HOUR_OF_DAY),
