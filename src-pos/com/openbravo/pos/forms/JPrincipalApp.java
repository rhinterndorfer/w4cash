...
public class JPrincipalApp extends javax.swing.JPanel implements AppUserView {
	...
	public class ScriptSubmenu {
		...
		public void addChangePasswordAction() {
			menudef.addMenuItem(
					new ChangePasswordAction("/com/openbravo/images/yast_security.png", "Menu.ChangePassword"));
		}
		...
		public void actionPerformed(ActionEvent evt) {
			...
			if (sNewPassword != null) {
				try {
					...
					JMessageDialog.showMessage(m_appview, JPrincipalApp.this, new MessageInf(MessageInf.SGN_WARNING,
		...
		if (m_appuser.hasPermission(sTaskClass)) {
			...
			if (m_jLastView == null || (m_jMyView != m_jLastView && m_jLastView.deactivate())) {
				...
				try {
					...
					setMenuVisible(getBounds().width > 999);
					//setMenuVisible(false);
					...
					JConfirmDialog.showError(m_appview, this, "", AppLocal.getIntString("message.notactive"), e);
				}
			}
		} else {
			// No hay permisos para ejecutar la accion...
			JConfirmDialog.showError(m_appview, this, AppLocal.getIntString("message.notpermissions"),
	...
	public void executeTask(String sTaskClass) {
		...
		if (m_appuser.hasPermission(sTaskClass)) {
			try {
				...
				JMessageDialog.showMessage(m_appview, JPrincipalApp.this,
