...
public class FilterEditor<T> extends BasicComboBoxEditor {
    ...
    private void enableEditingMode() {
        editing = true;
        filterLabel.setFont(displayFond.deriveFont(Font.PLAIN));
        editingChangeListener.accept(true);
    }

    public void reset() {
        if (editing) {
            filterLabel.setFont(displayFond);
